var Gantt = (function () {
  ...
  class o {
    ...
    date_changed() {
      let t = !1;
      const { new_start_date: s, new_end_date: i } =
  ...
  class p {
    ...
    make_grid_highlights() {
      ...
      if (this.view_is(d.DAY)) {
        // 日视图：高亮当前天
        n("rect", {
          x: (e.diff(today, this.gantt_start, "hour") / this.options.step) * this.options.column_width,
          y: 0,
      ...
      } else if (this.view_is(d.WEEK)) {
        // 周视图：高亮当前周
        const startOfWeek = e.start_of(today, "day");
        const dayOfWeek = startOfWeek.getDay();
        const adjustedStartOfWeek = e.add(startOfWeek, -dayOfWeek, "day");
        const weekWidth = this.options.column_width;
        const x = (e.diff(adjustedStartOfWeek, this.gantt_start, "hour") / this.options.step) * this.options.column_width;
      ...
      } else if (this.view_is(d.MONTH)) {
        // 月视图：高亮当前月
        const startOfMonth = e.start_of(today, "month");
        const daysInMonth = e.get_days_in_month(startOfMonth);
        const monthWidth = (daysInMonth * this.options.column_width) / 30;
        const x = (e.diff(startOfMonth, this.gantt_start, "day") * this.options.column_width) / 30;
